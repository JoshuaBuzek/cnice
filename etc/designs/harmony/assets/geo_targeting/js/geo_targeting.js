jQuery.cookie = function (name, value, options) {
  if (typeof value != 'undefined') {
      ...
      var secure = options.secure ? ';'+options.secure : '';
      var sameSiteValue = options.sameSiteValue ? '; SameSite='+options.sameSiteValue : '';
      document.cookie = [name, '=', encodeURIComponent(value), expires, path, domain, sameSiteValue, secure].join('');
  } else {
...
function(global) {
  var GeoCheck = function () {
    ...
    this.isSameSiteCompatible = canHandleSameSiteAttribute(navigator.userAgent);
    this.sameSiteValue = this.isSameSiteCompatible ? "None" : "" ; /* Value of SameSite cookie parameter to be set */
     this.secureValueValue = this.isSameSiteCompatible ? "Secure" : ""; /* Value of Secure cookie parameter to be set */
    return this
  };
  ...
  GeoCheck.prototype.noThanksCallback = function () {
    $.cookie(this.cookie_name, 'US', this.exdays, this.sameSiteValue, this.secureValue);
    $.modal.close();
  ...
  GeoCheck.prototype.goToSiteCallback = function () {
    $.cookie(this.cookie_name, this.country_code, this.exdays, this.sameSiteValue, this.secureValue);
  ...
  GeoCheck.prototype.onLoadCallback = function () {
    ...
    if (this.country_code !== 'US') {
      if(dataHash.ip) {
        $.ajax({
          url: Intuit.Utils.Constants.SBGMktgServiceEndpoint + "/v1/geoip/full",
          type: "POST",
          data: JSON.stringify(dataHash),
          beforeSend: function(xhr) {
            xhr.setRequestHeader( "Authorization", Intuit.Utils.Constants.SBGMktgServiceKey );
        ...
        $.ajax({
          url: Intuit.Utils.Constants.SBGMktgServiceEndpoint + "/v1/geoip",
          type: "GET",
          beforeSend: function(xhr) {
            xhr.setRequestHeader( "Authorization", Intuit.Utils.Constants.SBGMktgServiceKey );
          },
